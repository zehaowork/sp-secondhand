...
const Result: React.FC<any> = () => {
  ...
  const [itemRecommendationList, setItemRecommendationList] = useState<
    Array<Item>
  >([]);
  ...
  useEffect(() => {
    setKeyword(
      $instance.router?.params.keyword === undefined
        ? ""
        : $instance.router?.params.keyword
    ); // 继承搜索页面的关键字;
  ...
  const renderSortActions = sortOptions.map((option) => (
    <AtActionSheetItem
      onClick={() => {
      ...
      {itemRecommendationList.length !== 0 && (
        <React.Fragment>
          ...
          <GoodsList
            ...
            keyword={keyword}
            isFavouritesPage={false}
            itemList={itemRecommendationList}
          />
        </React.Fragment>
      )}
