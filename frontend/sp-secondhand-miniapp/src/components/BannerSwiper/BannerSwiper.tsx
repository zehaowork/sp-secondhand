...
const BannerSwiper: React.FC<Props> = (props)=>{
    ...
    //渲染函数
    const renderImage = props.imageList.map(banner =>  <SwiperItem 
    key={banner.id} 
    > 
        <Image onClick={()=>{toWebView(banner.link)}} mode='aspectFill'  src={'http://120.79.59.51:8087/'+banner.imgUrl} className={s.image} />
    </SwiperItem>)
